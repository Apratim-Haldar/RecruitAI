...
interface Props {
  applications: applicantType[];
  loading: boolean;
  reload: () => void; // Fixed typo in prop name
}

export function ApplicantsTable({ applications, loading, reload }: Props) {
  ...
  const updateStatus = async (applicationId: string, status: string) => {
    try {
      ...
      console.log("Error updating status at applicants-table.tsx: ", error);
    }
  };
                ...
                {applicant.status === "applied" && (
                  <TableCell onClick={(e) => e.stopPropagation()}>
                    <div className="flex items-center gap-2">
                      <Button
                        onClick={() => 
                          handleActionClick(applicant._id, applicant.firstName, applicant.lastName, "shortlisted")
                        }
                        size="sm"
                        variant="outline"
